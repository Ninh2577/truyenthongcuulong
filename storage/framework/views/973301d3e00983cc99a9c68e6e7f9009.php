<?php $__env->startSection('title', 'Quay Phim Sự Kiện & Team Building - Truyền Thông Cửu Long'); ?>
<?php $__env->startSection('meta_description', 'Xưởng phim sản xuất TVC quảng cáo 4K, phim tài liệu doanh nghiệp và video viral chuyên nghiệp với trang thiết bị điện ảnh hiện đại.'); ?>
    ...
    <section class="relative w-full overflow-hidden pt-32 pb-14 lg:pt-36 lg:pb-20 border-b border-slate-200/80 bg-surface-low bg-dot-grid-subtle">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <nav class="flex items-center gap-2 text-xs font-headline text-slate-400 mb-6" aria-label="Breadcrumb">
                ...
                <a href="<?php echo e(route('services.index')); ?>" class="hover:text-primary transition-colors">Dịch vụ</a>
                <span class="text-slate-600">/</span>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
                <div class="lg:col-span-8 flex flex-col gap-5">
                    ...
                    <div class="flex flex-wrap items-center gap-4 pt-3">
                        <a href="<?php echo e(route('contact')); ?>?service=media" class="px-6 py-3 rounded-xl bg-gradient-to-r from-primary to-accent-coral text-white font-headline text-xs sm:text-sm font-bold shadow-lg shadow-primary/25 hover:brightness-110 transition-all flex items-center gap-2">
                ...
                <div class="lg:col-span-4">
                    <div class="scroll-reveal-right transition-all duration-700 ease-out transform p-6 rounded-3xl bg-white border border-slate-200 shadow-xl flex flex-col gap-5 opacity-0 translate-x-8">
                        ...
                        <div class="pt-2">
                            <a href="<?php echo e(route('pricing')); ?>" class="w-full py-2.5 rounded-xl bg-surface-low hover:bg-slate-100 text-navy-base font-headline text-xs font-bold transition-all text-center block border border-slate-200">
    ...
    <section class="w-full bg-surface bg-dot-grid-subtle py-16 lg:py-20 border-b border-slate-200/80">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 lg:mb-12">
                ...
                <a href="<?php echo e(route('projects.index')); ?>?group=media" class="inline-flex items-center gap-2 text-primary font-headline text-sm font-bold hover:underline">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $mediaCaseStudies; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $case): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div class="opacity-0 translate-y-8 scroll-reveal rounded-3xl overflow-hidden bg-white border border-slate-200/90 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 flex flex-col justify-between group" style="transition-delay: <?php echo e($loop->index * 150); ?>ms;" x-data="{ playing: false, hasVideo: <?php echo e($case->video_url ? 'true' : 'false'); ?> }" @mouseenter="if(hasVideo) { playing = true; $refs.vid.play(); }" @mouseleave="if(hasVideo) { playing = false; $refs.vid.pause(); $refs.vid.currentTime = 0; }">
                    <div class="h-64 w-full relative overflow-hidden bg-navy-base">
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($imageUrl): ?>
                            <img src="<?php echo e($imageUrl); ?>" alt="<?php echo e($case->title); ?>" class="absolute inset-0 w-full h-full object-cover transition-opacity duration-500" :class="playing ? 'opacity-0' : 'opacity-100'">
                        <?php else: ?>
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($case->video_url): ?>
                        <!-- Video Preview (Muted, Loop, Playsinline) -->
                        <video x-ref="vid" class="absolute inset-0 w-full h-full object-cover transition-opacity duration-500" :class="playing ? 'opacity-100 scale-105' : 'opacity-0 scale-100'" muted loop playsinline preload="none" poster="<?php echo e($imageUrl ?? ''); ?>">
                            <source src="<?php echo e($case->video_url); ?>" type="video/mp4">
                        </video>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        ...
                        <!-- Play Button -->
                        <button type="button" @click="openVideo('<?php echo e($case->video_url); ?>')" class="absolute inset-0 m-auto w-14 h-14 rounded-full bg-primary text-white flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform cursor-pointer z-10">
                        ...
                        <div class="absolute top-3 left-3 z-10">
                            <span class="px-3 py-0.5 rounded-full bg-navy-base/80 backdrop-blur-md text-amber-400 font-mono text-xs font-bold border border-amber-400/30">
                                <?php echo e($case->client_name ?: 'Đối Tác Doanh Nghiệp'); ?>
                    ...
                    <div class="p-5 flex flex-col gap-2.5">
                        ...
                        <p class="font-body text-xs text-slate-600 line-clamp-2 leading-relaxed">
                            <?php echo e($case->summary ?: 'Chiến dịch hình ảnh chuyên nghiệp do ekip Truyền Thông Cửu Long trực tiếp bấm máy và hoàn thiện hậu kỳ.'); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\truyenthongcuulong-laravel\resources\views/services/media.blade.php ENDPATH**/ ?>