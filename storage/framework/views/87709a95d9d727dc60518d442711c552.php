<?php $__env->startSection('title', 'Kho Giao Diện Website Đa Ngành Chuẩn SEO - Truyền Thông Cửu Long TechLab'); ?>
<?php $__env->startSection('meta_description', 'Khám phá 39+ mẫu giao diện website chuẩn SEO, tương thích mọi thiết bị di động, điểm PageSpeed 98/100, sẵn sàng triển khai vận hành trong 48 giờ.'); ?>
    ...
    <section class="relative pt-32 pb-12 lg:pt-36 lg:pb-16 bg-surface-low bg-dot-grid-subtle border-b border-slate-200/80 overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <!-- Search Input -->
                <form action="<?php echo e(route('templates.index')); ?>" method="GET" class="w-full md:w-80 shrink-0">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(request('industry')): ?>
                        <input type="hidden" name="industry" value="<?php echo e(request('industry')); ?>">
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    <div class="relative flex items-center group">
                        <input type="text" name="q" value="<?php echo e(request('q')); ?>" placeholder="Tìm tên hoặc mã mẫu web..." 
    ...
    <section class="py-6 bg-surface bg-dot-grid-subtle border-b border-slate-200/80">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between gap-4 mb-3">
                <span class="font-mono text-xs font-bold text-slate-600 uppercase tracking-wider">LỌC THEO NGÀNH NGHỀ (<?php echo e($industries->count()); ?> NHÓM NGÀNH):</span>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedIndustry || request('q')): ?>
                <a href="<?php echo e(route('templates.index')); ?>" class="text-xs font-mono font-bold text-amber-600 hover:underline flex items-center gap-1">
            ...
            <div class="flex items-center gap-2 overflow-x-auto pb-4 no-scrollbar scroll-smooth snap-x">
                <a href="<?php echo e(route('templates.index')); ?>" 
                    class="snap-start px-4 py-2 rounded-full text-xs font-headline font-bold whitespace-nowrap transition-all duration-300 <?php echo e(empty($selectedIndustry) ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/25' : 'bg-transparent text-slate-600 hover:text-amber-600 hover:border-amber-400/50 border border-slate-200 shadow-sm'); ?>">
                    Tất cả ngành nghề (<?php echo e($templates->total()); ?>)
                </a>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $industries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ind): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('templates.index', ['industry' => $ind->slug])); ?>" 
                    class="snap-start px-4 py-2 rounded-full text-xs font-headline font-bold whitespace-nowrap transition-all duration-300 <?php echo e($selectedIndustry === $ind->slug ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/25' : 'bg-transparent text-slate-600 hover:text-amber-600 hover:border-amber-400/50 border border-slate-200 shadow-sm'); ?>">
                    <?php echo e($ind->name); ?>
    ...
    <section class="py-12 lg:py-16 bg-surface bg-dot-grid-subtle border-b border-slate-200/80">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $templates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php
                ...
                <div class="group rounded-3xl overflow-hidden bg-white border border-slate-200 hover:border-amber-400/50 shadow-sm hover:shadow-2xl hover:shadow-amber-500/10 hover:-translate-y-2 transition-all duration-300 flex flex-col justify-between relative">
                    ...
                    <div>
                        ...
                        <div class="w-full bg-slate-50 px-4 py-2.5 flex items-center gap-1.5 border-b border-slate-100 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
                            ...
                            <span class="ml-2 text-[9px] font-mono text-slate-400 truncate max-w-[120px]"><?php echo e($item->slug); ?>.preview</span>
                        </div>

                        <!-- Thumbnail Preview Area -->
                        <div class="aspect-[4/3] sm:aspect-video w-full relative overflow-hidden bg-slate-50 group/img">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($thumbSrc): ?>
                                <img src="<?php echo e($thumbSrc); ?>" alt="<?php echo e($item->title); ?>" class="w-full h-full object-cover group-hover/img:scale-105 transition-transform duration-700" loading="lazy" decoding="async">
                            <?php else: ?>
                            ...
                            <div class="absolute inset-0 bg-navy-base/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-sm z-20">
                                <button type="button" @click.prevent="openPreview('<?php echo e(addslashes($item->title)); ?>', '<?php echo e($item->slug); ?>', '<?php echo e($previewSrc); ?>')" 
                        ...
                        <div class="p-5 sm:p-6 flex flex-col gap-3">
                            <div class="flex items-center gap-2 flex-wrap relative z-30">
                                <span class="text-[9px] font-mono font-bold text-slate-500 uppercase tracking-wider bg-slate-100 px-2 py-0.5 rounded border border-slate-200">
                                    <?php echo e($item->category ? $item->category->name : 'Web Architecture'); ?>
                            ...
                            <h3 class="font-headline text-base sm:text-lg font-bold text-navy-base group-hover:text-primary transition-colors line-clamp-2 leading-tight">
                                <a href="<?php echo e(route('blog.resolve', $item->slug)); ?>" class="focus:outline-none before:absolute before:inset-0 before:z-10"><?php echo e($item->title); ?></a>
                            </h3>

                            <p class="font-body text-xs sm:text-sm text-slate-500 line-clamp-2 leading-relaxed">
                                <?php echo e($item->summary ?: 'Giao diện thiết kế độc quyền, tối ưu điểm SEO & tốc độ tải trang, mang lại trải nghiệm khách hàng vượt trội.'); ?>
                    ...
                    <div class="px-5 sm:px-6 mb-6 sm:mb-8 mt-auto relative z-30 flex flex-wrap items-center justify-between gap-3 pt-2">
                        ...
                        <a href="<?php echo e(route('contact', ['service' => 'Template: ' . $item->title])); ?>" 
                ...
                <div class="col-span-3 text-center py-20 bg-white rounded-3xl border border-slate-200">
                    ...
                    <a href="<?php echo e(route('templates.index')); ?>" class="mt-4 inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-amber-400 text-slate-950 font-headline text-xs font-bold hover:bg-amber-300 transition-all">
        ...
        <div @click.outside="previewModal = false" class="w-full max-w-5xl h-[85vh] bg-[#0F172A] rounded-3xl border border-slate-700 shadow-2xl overflow-hidden flex flex-col">
            ...
            <div class="h-14 px-6 bg-[#0B132B] border-b border-slate-800 flex items-center justify-between shrink-0">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a :href="'<?php echo e(route('contact')); ?>?service=' + encodeURIComponent('Template: ' + previewTitle)" 
...
<script type="application/ld+json">
{
    ...
    "url": "<?php echo e(route('templates.index')); ?>",
    "provider": {
        "@type": "Organization",
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\truyenthongcuulong-laravel\resources\views/templates/index.blade.php ENDPATH**/ ?>