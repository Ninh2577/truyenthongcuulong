<?php $__env->startSection('title', 'Tạp Chí Truyền Thông & Công Nghệ - Truyền Thông Cửu Long'); ?>
<?php $__env->startSection('meta_description', 'Khám phá kiến thức chuyên sâu về Sản xuất TVC điện ảnh, Thiết kế Web/App chịu tải cao, Chiến lược Digital Marketing và Thư viện tài nguyên tải về.'); ?>
...
<div class="w-full bg-surface bg-dot-grid-subtle pt-28 pb-20 border-b border-slate-200/60">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 pb-8 border-b border-slate-200">
            ...
            <div class="w-full md:w-80 relative" x-data="{
                ...
                search() {
                    ...
                    this.loading = true;
                    fetch('<?php echo e(route('api.search-posts')); ?>?q=' + encodeURIComponent(this.query))
                        .then(res => res.json())
                        .then(data => {
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-4 mb-10 no-scrollbar">
            ...
            <a href="<?php echo e(route('blog.resolve', $cat->slug)); ?>" 
                ...
                <span><?php echo e($cat->name); ?> (<?php echo e($cat->posts_count); ?>)</span>
            </a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
        </div>

        <!-- Featured Big Magazine Banner (Only on first page without search) -->
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($featuredPost && !request('q') && !request('page')): ?>
        <div class="mb-14 rounded-3xl overflow-hidden bg-navy-base text-white border border-slate-700/80 shadow-[0_20px_50px_rgba(7,15,30,0.2)] grid grid-cols-1 lg:grid-cols-12 group">
            <div class="lg:col-span-7 relative h-72 sm:h-96 lg:h-auto overflow-hidden bg-black">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($featuredPost->thumbnail): ?>
                    <img src="<?php echo e($featuredPost->thumbnail_url); ?>" alt="<?php echo e($featuredPost->title); ?>" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700 opacity-90">
                <?php else: ?>
            ...
            <div class="lg:col-span-5 p-8 lg:p-10 flex flex-col justify-between">
                <div class="flex flex-col gap-4">
                    <div class="flex items-center gap-3 text-xs font-mono text-slate-400">
                        <span class="text-amber-400 font-bold uppercase"><?php echo e($featuredPost->category?->name ?? 'Tin tức'); ?></span>
                        <span>•</span>
                        <span><?php echo e($featuredPost->published_at ? $featuredPost->published_at->format('d/m/Y') : ''); ?></span>
                        <span>•</span>
                        <span><?php echo e($featuredPost->views); ?> views</span>
                    </div>
                    <h2 class="font-headline text-2xl sm:text-3xl font-extrabold text-white group-hover:text-amber-300 transition-colors leading-tight">
                        <a href="<?php echo e(route('blog.resolve', $featuredPost->slug)); ?>"><?php echo e($featuredPost->title); ?></a>
                    </h2>
                    <p class="font-body text-sm text-slate-300 line-clamp-3 leading-relaxed">
                        <?php echo e($featuredPost->summary); ?>
                ...
                <div class="pt-6 mt-6 border-t border-white/10 flex items-center justify-between">
                    ...
                    <a href="<?php echo e(route('blog.resolve', $featuredPost->slug)); ?>" class="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-white/10 hover:bg-primary text-white font-headline text-xs font-bold transition-all group-hover:translate-x-1">
            ...
            <div class="lg:col-span-8 flex flex-col">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-7 mb-10">
                    ...
                    <div class="sm:col-span-2 text-center py-16 bg-white rounded-3xl border border-slate-200">
                        ...
                        <a href="<?php echo e(route('blog.index')); ?>" class="mt-4 inline-flex items-center gap-1 px-4 py-2 rounded-full bg-primary text-white font-headline text-xs font-bold">Xem tất cả bài viết</a>
                    </div>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                </div>
            ...
            <div class="lg:col-span-4 flex flex-col gap-8">
                ...
                <div class="p-6 rounded-3xl bg-white border border-slate-200/90 shadow-xs flex flex-col gap-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <a href="<?php echo e(route('blog.resolve', $cat->slug)); ?>" class="flex items-center justify-between p-2.5 rounded-xl hover:bg-orange-50/60 transition-colors group">
                ...
                <div class="p-7 rounded-3xl bg-gradient-to-br from-navy-base to-[#0a1c38] text-white border border-white/10 shadow-lg flex flex-col gap-4 relative overflow-hidden">
                    ...
                    <a href="<?php echo e(route('contact')); ?>" class="mt-2 inline-flex items-center justify-center gap-2 py-3 px-5 rounded-full bg-gradient-to-r from-primary to-accent-amber text-white font-headline text-xs font-bold shadow-md hover:scale-[1.02] transition-transform">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\truyenthongcuulong-laravel\resources\views/blog/index.blade.php ENDPATH**/ ?>